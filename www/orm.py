...
#目的是为了让每个HTTP请求都能从连接池中直接获取数据库连接，避免了频繁关闭和打开数据库连接
async def  create_pool(loop,**kw):
	logging.info('创建连接池...')
	#声明变量__pool是一个全局变量，如果不加声明，__pool就会默认为一个私有变量，不能被其他函数引用
	global __pool 

	#调用一个协程来创建全局连接池，create_pool的返回值是一个pool实例对象
	__pool=await aiomysql.create_pool(
		#下面就是创建数据库连接需要用到的一些参数，从**kw参数中取出
		#因为kw是一个字典，而dict.get(key,default=None)函数返回指定键的值，如果值不在字典中则返回默认值
		...
		loop=loop #传递消息循环对象，用于异步执行（不是很明白）
		)


#============================================sql函数处理区
#select和execute方法是实现其他Model类中SQL语句经常要用的方法
#为什么要分成select和execute，因为增删改都只影响数据库表中的一行数据，而查找可能会有多条结果

#将执行sql的代码封装进select函数，调用的时候只要传入sql和sql所需要的一些参数就好
#参数说明   sql：即为sql语句，args表示要搜索的参数，size用于指定最大的查询数量，不指定将返回所有查询结果
async def select(sql,args,size=None):
	log(sql,args)

	#声明全局变量，这样才能引用ceate_pool函数创建的__pool变量
	global __pool

	#从连接池中获得一个数据库连接，用with语句可以封装清理（关闭conn）和处理异常工作
	async with __pool.get()  as conn:
		...
		async with conn.cursor(aiomysql.DictCursor)  as  cur:
			...
			#fetchone(),fetchmany(),fetchall()的用法
			if size:
...
async def  execute(sql,args,autocommit=True):
	log(sql)
	#这一段反复出现的if not autocommit还不是很懂
	async with __pool.get() as conn:
		if not autocommit:
			await conn.begin()
		try:
...
class ModelMetaclass(type):
	...
	for k,v in attrs.items():
		if isinstance(v,Field):
			...
			#先判断找到的映射是不是主键
			if v.primary_key:
				if primaryKey:     #若主键已存在，又找到一个主键，将报错，因为每张表有且仅有一个主键
	...
	#定义域中的key值已经添加到fields里，就要在attrs中删除，避免重名导致运行时错误
	for k in mappings.keys():
		attrs.pop(k)
	#将非主键的属性变形，放入escaped_fields，方便sql语句的书写
	escaped_fields=list(map(lambda f:'`%s`' %f,fields))	 #查一下lambda的用法,注意'`%s`'的写法
	...
	def getValueOrDefault(self,key) :
		...
		if value is None :
			...
			if field.default is not None :
				#如果field的default属性是callable（可被调用的），就给value 它调用后的值，如果不可被调用直接返回这个值
...
async def  findAll (cls,where=None,args=None,**kw) :
	...
	if args is None:
		args=[]	
	#如果有OrderBy参数就在sql语句中添加字符串OrderBy参数和OrderBy
...
async def save(self) :
	...
	if rows !=1:    #插入记录受影响的行数应该为1，如果不是1，那就错了
